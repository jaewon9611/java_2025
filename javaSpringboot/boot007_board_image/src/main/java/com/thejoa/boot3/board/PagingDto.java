...
public class PagingDto {
	...
	private int pagetotal;   //#3) 총페이지수 123/10  =  13   12페이지+1글
	...
	public PagingDto(int listtotal, int pstartno) { // pstartno 0 1 2 3
		...
		// partno=10 이라고하면 현재페이지번호는 2
		// partno=40 이라고하면 현재페이지번호는 5
		this.current = (this.pstartno/this.onepagelist) +1;    //#6) 현재페이지번호
		
		//시작)  11  15  20 (20을 11, 15를 11, 11을 11 -> 앞자리는 1 -> 1*10 +1)
		this.start = ((this.current-1)/bottomlist)*this.bottomlist +1; //#7) 시작페이지번호
