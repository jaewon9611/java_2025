...
public class BoardService {
	...
	/* PAGING */
	/* PAGING */
	// 1 : 최신글 10 order by bno desc 0 limit 0,10 0번째부터 10개
	// 2 : 그다음최신글 10 order by bno desc 0 limit 10,10 10번째부터 10개
	...
	public int update(MultipartFile file, Board board) {
		// file Upload
		// 새로운 파일이 올라왔는지 확인.
		String save = board.getBfile(); // edit.html 먼저 파일경로
		...
		return boardRepository.updateByIdAndBpass(board.getId(), board.getBpass(), board.getBtitle(),
				board.getBcontent(), board.getBfile());
	}
